...
struct pf_length_modifier
{
    const char * name;
    enum pf_length length;
};

struct pf_length_modifier length_modifiers [] =
{
...
int printf(const char* __restrict format, ...)
{
    ...
    while ( *format != '\0' )
    {
        ...
        enum pf_length length = LENGTH_DEFAULT;
        ...
        {
            format++;
            const char* b = va_arg(parameters, int) ? (const char*)"true" : (const char*)"false";
            print(b, strlen(b));
        }
        else
        {
